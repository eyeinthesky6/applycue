...
- Application attempt receipts are append-only evidence; they do not replace the tracker. A confirmed finish must reconcile the exact company/role tracker row to `Applied` and rebuild the derived index before recording the final receipt. The receipt's `firstConfirmedApplication` boolean lets the agent offer one optional public feedback link after the first confirmed success; it does not change lifecycle state or add a submission gate. Other outcomes remain attempt evidence and never imply `Applied`.
