...
7. Before writing each role CV, it builds a light employer success brief, chooses the primary role family, and asks focused questions when useful work may simply be missing from the old CV. A confirmed recollection can be used without an old document or exact metric; the agent must not invent precision. When useful, it saves a confirmed reusable career spine in local `candidate-positioning.md`.
