...
// relative to applycuePath. Links are normally relative to the tracker
...
func resolveReportPath(applycuePath, trackerPath, link string) string {
	resolved := filepath.Join(filepath.Dir(trackerPath), link)
	if _, err := os.Stat(resolved); err != nil {
		legacy := filepath.Join(applycuePath, link)
		if _, err2 := os.Stat(legacy); err2 == nil {
			resolved = legacy
		}
	}
	if rel, err := filepath.Rel(applycuePath, resolved); err == nil {
		return rel
	}
	return link
}
...
func ParseApplications(applycuePath string) []model.CareerApplication {
	filePath := filepath.Join(applycuePath, "applications.md")
	content, err := os.ReadFile(filePath)
	if err != nil {
		// Fallback: try data/ subdirectory
		filePath = filepath.Join(applycuePath, "data", "applications.md")
		content, err = os.ReadFile(filePath)
		if err != nil {
			return nil
		}
	...
	for _, line := range lines {
		...
		// back to a applycuePath-relative path, which is what every
		...
		if rm := reReportLink.FindStringSubmatch(at("report")); rm != nil {
			...
			app.ReportPath = resolveReportPath(applycuePath, filePath, rm[2])
		}
	...
	batchURLs := loadBatchInputURLs(applycuePath)
	reportNumURLs := loadJobURLs(applycuePath)

	for i := range apps {
		if apps[i].ReportPath == "" {
			continue
		}
		fullReport := filepath.Join(applycuePath, apps[i].ReportPath)
	...
	enrichFromScanHistory(applycuePath, apps)

	// Strategy 5: company name fallback from batch-input.tsv
	enrichAppURLsByCompany(applycuePath, apps)

	return apps
}

// loadBatchInputURLs reads batch-input.tsv and returns a map of batch ID -> job URL.
func loadBatchInputURLs(applycuePath string) map[string]string {
	inputPath := filepath.Join(applycuePath, "batch", "batch-input.tsv")
...
func loadJobURLs(applycuePath string) map[string]string {
	...
	statePath := filepath.Join(applycuePath, "batch", "batch-state.tsv")
...
func enrichFromScanHistory(applycuePath string, apps []model.CareerApplication) {
	scanPath := filepath.Join(applycuePath, "scan-history.tsv")
...
func enrichAppURLsByCompany(applycuePath string, apps []model.CareerApplication) {
	inputPath := filepath.Join(applycuePath, "batch", "batch-input.tsv")
	inputData, err := os.ReadFile(inputPath)
	if err != nil {
		return
...
// Aliases match states.yml -- keep in sync with ApplyCue/states.yml
...
func LoadReportSummary(applycuePath, reportPath string) (archetype, tldr, remote, comp string) {
	fullPath := filepath.Join(applycuePath, reportPath)
...
func UpdateApplicationStatus(applycuePath string, app model.CareerApplication, newStatus string) error {
	filePath := filepath.Join(applycuePath, "applications.md")
	content, err := os.ReadFile(filePath)
	if err != nil {
		filePath = filepath.Join(applycuePath, "data", "applications.md")
