...
type PDFManifestEntry struct {
	ReportNumber string
	PDFPath      string // relative to the ApplyCue root
	HTMLPath     string // relative to the ApplyCue root, "" if unknown
...
func LoadPDFEntriesByPath(applycuePath string) map[string]PDFManifestEntry {
	byPath := make(map[string]PDFManifestEntry)
	raw, err := os.ReadFile(filepath.Join(applycuePath, "data", "pdf-index.tsv"))
	if err != nil {
		return byPath
	}
...
// LoadPDFManifest reads data/pdf-index.tsv under applycuePath. A missing
...
func LoadPDFManifest(applycuePath string) PDFManifest {
...
// ResolvePDFs returns candidate PDF paths (relative to applycuePath) for an
...
func ResolvePDFs(applycuePath string, app model.CareerApplication, manifest PDFManifest) []string {
	if entry, ok := manifest.Lookup(app); ok {
		if _, err := os.Stat(filepath.Join(applycuePath, filepath.FromSlash(entry.PDFPath))); err == nil {
	...
	globbed, err := filepath.Glob(filepath.Join(applycuePath, "output", "cv-*.pdf"))
	if err != nil {
		return nil
	}
	...
	for _, p := range globbed {
		...
		if matchesCompanySlug(base, slug) {
			if rel, err := filepath.Rel(applycuePath, p); err == nil {
				matches = append(matches, filepath.ToSlash(rel))
			}
		}
	}

	sortPDFsNewestFirst(applycuePath, matches)
	return matches
}
...
func sortPDFsNewestFirst(applycuePath string, paths []string) {
	...
	mtimeOf := func(p string) int64 {
		info, err := os.Stat(filepath.Join(applycuePath, filepath.FromSlash(p)))
		if err != nil {
			return 0
		}
...
func ResolveHTML(applycuePath string, app model.CareerApplication) (htmlPath, pdfPath string) {
	slug := kebabCase(app.Company)
	if slug == "" {
		return "", ""
	}
	globbed, err := filepath.Glob(filepath.Join(applycuePath, "output", "cv-*.html"))
	if err != nil {
		return "", ""
	}
	var matches []string
