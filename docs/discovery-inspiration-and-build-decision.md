...
Build ApplyCue as its own product and engine.

Use Career OS and OSS projects as references for product patterns, source adapters, and safety checks. Do not import Career OS code or make ApplyCue a thin wrapper around it.

Current answer: do not use Career OS itself as the runtime base.

Reason:

- Career OS is useful proof that the local loop can work.
- Career OS has good source-filtering discipline that ApplyCue should learn from.
- ApplyCue's product contract is different: chat-first, CV-to-offer, batch automation by default, user assets outside the repo, engine-owned truth checks, and apply policy gates.
- If ApplyCue depends on Career OS as the core, the product becomes "Career OS plus features" instead of an independent agent engine.

## What Career OS Proves

Career OS is useful because it has a working loop:
...
Career OS is a strong local power-user product. ApplyCue should become a CV-to-offer agent with automation as the default and user oversight through policy.
...
| --- | --- | --- |
| Career OS | MIT, working local product | Use as product inspiration only. Do not copy or depend on it. |
...
- Career OS local repo: `C:\Projects\career-ops`
...
Career OS gets relevance partly from `portals.yml` filters:
...
This is the first direct Career OS parity move: better source quality before expensive evaluation, without importing Career OS as ApplyCue's engine or hardcoding one user's market.
...
## What ApplyCue Was Doing Worse Than Career OS
...
Career OS has a useful liveness habit:
...
Career OS has an important scanner habit: it remembers URLs it has already seen, records dead or invalid postings, and avoids feeding the same job back into expensive evaluation.
...
This gives ApplyCue Career OS-style repeat control without moving user data into the repo and without hiding user-pasted manual jobs.

## Repost And Stale Opening Signals

Status: implemented on 2026-07-07.

Career OS detects repost clusters by grouping scan history by company, fuzzy-matching role titles, and looking for multiple URLs inside a recent window. ApplyCue now keeps that pattern as a warning signal:
...
Career OS has a useful pattern analyzer that reads tracker status and report summaries to find what is working. ApplyCue keeps the same lesson in structured state:
...
Career OS has a useful `scan:full` pattern:
...
- it follows the Career OS learning without copying Career OS code
