#!/usr/bin/env node
/**
 * gemini-eval.mjs — Gemini-powered Job Offer Evaluator for ApplyCue
 *
...
const PATHS = {
  ...
  evaluate:    join(ROOT, '.claude', 'skills', 'ApplyCue', 'SKILL.md'),
  cv:          join(ROOT, 'cv.md'),
...
if (args.length === 0 || args[0] === '--help' || args[0] === '-h') {
  console.log(`
╔══════════════════════════════════════════════════════════════════╗
║           ApplyCue — Gemini Evaluator (free-tier)             ║
╚══════════════════════════════════════════════════════════════════╝
...
function validateEvaluationShape(text) {
  ...
  if (issues.length > 0) {
    throw new Error(`Gemini returned an invalid ApplyCue report: ${issues.join('; ')}`);
  }
}
...
// ---------------------------------------------------------------------------
console.log('\n' + '═'.repeat(66));
console.log('  ApplyCue EVALUATION — powered by Google Gemini');
