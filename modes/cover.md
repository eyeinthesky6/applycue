...
Works in two modes:
- **Slug mode:** `/applycue cover {slug}` — loads the existing evaluation report draft as a starting point
- **Paste mode:** `/applycue cover` or JD pasted directly — starts from scratch
...
When invoked as `/applycue cover {slug}`:
