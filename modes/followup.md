...
> "No active applications to follow up on. Apply to some roles first with `/applycue` and come back when they're aging."
...
- Suggest the user run `/applycue contacto {company}` to find the right person first
...
> - Trying a different contact via `/applycue contacto`
...
**To:** {email or "No contact found — run `/applycue contacto` first"}
