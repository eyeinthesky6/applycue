...
> Claude Code marketplace metadata). These plugins extend ApplyCue itself.
...
A plugin is a directory under `plugins/` (bundled, shipped with ApplyCue) or
...
ApplyCue is plain ESM with no build step, so the engine **cannot truly
...
- **Centralized infrastructure** the project would operate — hosted job
  ...
  [Where ApplyCue is going (#904)](https://github.com/eyeinthesky6/applycue/discussions/904) —
  not the open-core.
- **Auto-submitting / blind-applying** to jobs. ApplyCue is a decision-support
