// @ts-check
/**
 * plugins/_engine.mjs — the ApplyCue plugin engine.
 *
...
export const RESERVED_ENV = new Set([
  ...
  'OPENROUTER_API_KEY', 'APPLYCUE_MODEL',
  'OPENAI_API_KEY', 'OPENAI_BASE_URL', 'OPENAI_MODEL',
  'ANTHROPIC_API_KEY',
  'APPLYCUE_PORTALS', 'APPLYCUE_PROFILE',
...
export function validateManifest(m, dir, dirName) {
  ...
  if (entryAbs !== pluginDir && !entryAbs.startsWith(pluginDir + path.sep)) { warnSkip(label, `entry "${entry}" escapes the plugin directory`); return null; }
  ...
  if (m.skill !== undefined) {
    ...
    const skillAbs = path.resolve(pluginDir, m.skill);
    if (skillAbs !== pluginDir && !skillAbs.startsWith(pluginDir + path.sep)) { warnSkip(label, `skill "${m.skill}" escapes the plugin directory`); return null; }
