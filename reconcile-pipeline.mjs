 ...
 * next `/applycue pipeline` run both re-surface them, and they get evaluated
...
function resolveInsideRepo(inputPath, fallbackPath, flag) {
  ...
  try {
    repoReal = realpathSync(APPLYCUE);
...
const defaultPipeline = existsSync(join(APPLYCUE, 'data/pipeline.md'))
  ? join(APPLYCUE, 'data/pipeline.md')
  : join(APPLYCUE, 'pipeline.md');
const PIPELINE_FILE = resolveInsideRepo(argValue('--pipeline'), defaultPipeline, '--pipeline');
const STATE_FILE = resolveInsideRepo(argValue('--state'), join(APPLYCUE, 'batch/batch-state.tsv'), '--state');
